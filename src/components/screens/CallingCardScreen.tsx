  ...
  return (
    <div className="fixed inset-0 z-30 flex flex-col justify-between p-4 sm:p-6 md:p-8 select-none overflow-hidden bg-gradient-to-r from-black/95 from-0% via-black/80 via-35% to-transparent to-55% animate-in fade-in duration-200 pt-20 sm:pt-24 md:pt-26 pb-6">
      ...
      <div className="flex-1 flex flex-col justify-center items-start my-auto z-20 w-full max-w-2xl lg:max-w-[52%] pl-2 sm:pl-6 md:pl-8">
        ...
        <div className="w-full bg-[#FAF8F5] text-black border-4 sm:border-[5px] border-black shadow-[8px_8px_0px_#000000] -rotate-1 transition-transform duration-300 p-4 sm:p-5 relative">
          
          {/* Top Postcard Header: Notice of Intent & Phantom Stamp */}
          <div className="flex items-center justify-between pb-2 border-b-2 sm:border-b-4 border-black mb-3">
            <div className="flex items-center gap-2">
              <span className="bg-p5-crimson text-white font-p5Heading text-sm sm:text-lg px-2.5 py-0.5 border-2 border-black shadow-[2px_2px_0px_#000] -rotate-1">
                NOTICE OF INTENT // 予告状
              </span>
            </div>

            {/* Persona Stamp Badge */}
            <div className="border-2 border-dashed border-p5-crimson text-p5-crimson px-2 py-0.5 rotate-2 font-p5Heading text-[11px] sm:text-xs tracking-wider uppercase flex items-center gap-1 shadow-[2px_2px_0px_#E60012]">
          ...
          <form onSubmit={handleSend} className="space-y-3">
            ...
            <div className="flex items-center gap-2 font-p5Heading text-sm sm:text-base border-b-2 border-zinc-800 pb-1">
              <span className="text-black uppercase whitespace-nowrap text-xs sm:text-sm">TO SIR / MADAM:</span>
              <input
                type="text"
                value={recipient}
                onChange={e => setRecipient(e.target.value)}
                placeholder="Enter Recipient Organization..."
                className="flex-1 bg-transparent font-p5Heading text-p5-crimson text-sm sm:text-lg focus:outline-none uppercase placeholder:text-zinc-400"
              />
            </div>

            {/* Objective Pills */}
            <div>
              <span className="block font-p5Sub text-[9px] sm:text-[10px] text-zinc-600 uppercase tracking-wider mb-1">
                ...
                {CONTRACT_PRESETS.map(preset => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        px-2 py-1 font-p5Heading text-xs tracking-wider uppercase border-2 border-black transition-all
                        ${isActive
                          ? 'bg-p5-crimson text-white shadow-[2px_2px_0px_#000000] -rotate-1 font-bold'
            ...
            {/* Message Body */}
            <div>
              <span className="block font-p5Sub text-[9px] sm:text-[10px] text-zinc-600 uppercase tracking-wider mb-0.5">
                DECLARATION BODY:
              </span>
              <textarea
                rows={2}
                ...
                className="w-full bg-white border-2 border-black p-2.5 font-p5Body text-xs text-zinc-800 focus:outline-none focus:border-p5-crimson resize-none leading-relaxed"
              />
            </div>

            {/* Card Footer: Signature & Dispatch Button */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1 border-t-2 border-zinc-300">
              <div className="font-p5Heading text-[11px] sm:text-xs text-zinc-700">
                SIGNATURE: <span className="text-p5-crimson text-xs sm:text-sm">FERREL // THE ARCHITECT</span>
              </div>
              ...
              <button
                ...
                onMouseEnter={playHover}
                className={`
                  inline-flex items-center justify-center gap-1.5 px-4 py-1.5 font-p5Heading text-sm sm:text-base tracking-wider uppercase border-2 border-black shadow-[3px_3px_0px_#000000] transition-all
                  ${isSent
          ...
          <div className="mt-3 pt-2 border-t-2 border-dashed border-zinc-400 flex items-center justify-between flex-wrap gap-2 text-[11px] font-p5Mono">
            <button
              type="button"
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 bg-zinc-900 text-white px-2 py-0.5 hover:bg-p5-crimson transition-colors border border-black shadow-[2px_2px_0px_#000]"
            >
              <GithubIcon />
              <span>@FerrelHD</span>
            </a>
            ...
            <div className="hidden sm:flex items-center gap-1 text-[10px] text-zinc-500">
              <Radio className="size-2.5 text-p5-crimson" />
